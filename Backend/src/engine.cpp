...
EngineResult Engine::execute(
    ...
    try {
        ...
        for (size_t i = 0; i < n_bars; ++i) {
            ...
            int64_t dpart = val / 1000000LL;
            int64_t tpart = val % 1000000LL;
        ...
        for (const auto& [pool_key, ptr] : indicators_pool) {
            ...
            fast_pool[col]      = ptr;   // col_name curto → para lookups internos
            fast_pool[pool_key] = ptr;   // pool_key completo → para signal_refs
        }

        return Operation::run(
            header, fast_pool, n_bars,
            bar_dates, bar_times, bar_days,
            shared_signal_arrays, sim_params, exec_settings
        );
